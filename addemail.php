...
<?php
  ...
  $query = "INSERT INTO final_email (first_name, last_name, email)  VALUES ('$first_name', '$last_name', '$email')";
  mysqli_query($dbc, $query)
    or die('Error querying database.');

  	echo 'You have been added to Contact us. Click REMOVE if you wish to not be contacted.';
  ...
  { 
      ...
      echo '<td class= "email_class" >'. $row['email'] . '</td>';
 
 echo '<td>
    </form>
    	<form method = "post" action = "removeemail.php">
        	<input type = "hidden" name = "L_name" size = "30" value ="'. $row['last_name'].'"/><br />
            <input type = "hidden" name = "F_name" size = "30" value ="'. $row['first_name'].'"/><br />
            <input type = "hidden" name = "email" size = "30" value ="'. $row['email'].'"/><br />
