  ...
  if (isset($_POST['submit'])) {
    ...
    if (!$error) {
      if (!empty($first_name) && !empty($last_name) && !empty($gender) && !empty($birthdate) && !empty($city) && !empty($state)) {
        ...
        if (!empty($new_picture)) {
          $query = "UPDATE final_user SET first_name = '$first_name', last_name = '$last_name', gender = '$gender', " .
    ...
    $query = "SELECT first_name, last_name, gender, birthdate, city, state, picture FROM final_user WHERE user_id = '" . $_SESSION['user_id'] . "'";
